...
fun ProfileScreen(onOpenCertificates: () -> Unit) {
    ...
    LazyColumn(
        ...
        item {
            Text("Scholar Profile", style = MaterialTheme.typography.displayMedium, color = MaterialTheme.colorScheme.onBackground)
            RankBadge(
                rankLabel = progress.currentRank.name.replace('_', ' '),
                tierColor = MaterialTheme.colorScheme.primary,
                progress = progress.progressToNext,
                modifier = Modifier.padding(top = 12.dp)
            )
            Text(
                "${resolvedStats.totalXp} XP total · ${resolvedStats.currentStreakDays}-day streak (best ${resolvedStats.longestStreakDays})",
        ...
        item {
            Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)) {
                Row(Modifier.fillMaxWidth().padding(16.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                    Column {
                        Text("Biometric Lock", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurface)
                        Text("Require fingerprint or face unlock on launch", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
        ...
        item {
            Button(onClick = onOpenCertificates, modifier = Modifier.fillMaxWidth()) {
                Text("View Certificates of Honour")
            }
        }
        ...
        items(achievements) { achievement ->
            ...
            Card(colors = CardDefaults.cardColors(containerColor = if (unlocked) MaterialTheme.colorScheme.surface else MaterialTheme.colorScheme.surfaceVariant)) {
                Column(Modifier.padding(16.dp)) {
                    ...
                    Text(if (unlocked) "Unlocked" else "Locked", style = MaterialTheme.typography.labelMedium, color = if (unlocked) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurfaceVariant)
                }
            }
        }
    }
