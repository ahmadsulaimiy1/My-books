...
FACES = [
    # ── Arabic ──────────────────────────────────────────────────────────────
    # KUFIC for the name. Kufic is the monumental hand — the script of
    # inscriptions, of coinage, of the carved dedication over a doorway. It is
    # architectural rather than handwritten, which is why it reads at once as
    # authoritative and as modern, and why it sidesteps the question of whether
    # a scholar's name should be set in an everyday hand.
    ("Noto Kufi Arabic",  "family=Noto+Kufi+Arabic:wght@700", "arabic",    700, {"wght":700}),
    ("Noto Kufi Arabic",  "family=Noto+Kufi+Arabic:wght@700", "latin",     700, {"wght":700}),
    ("Noto Kufi Arabic",  "family=Noto+Kufi+Arabic:wght@400", "arabic",    400, {"wght":400}),
    ("Noto Kufi Arabic",  "family=Noto+Kufi+Arabic:wght@400", "latin",     400, {"wght":400}),
    # NASKH for everything that must be read at length: a letter body is not
    # the place for a display face, however monumental.
    ("Noto Naskh Arabic", "family=Noto+Naskh+Arabic:wght@400","arabic",    400, {"wght":400}),
    ("Noto Naskh Arabic", "family=Noto+Naskh+Arabic:wght@400","latin",     400, {"wght":400}),
    ("Noto Naskh Arabic", "family=Noto+Naskh+Arabic:wght@700","arabic",    700, {"wght":700}),
    ("Noto Naskh Arabic", "family=Noto+Naskh+Arabic:wght@700","latin",     700, {"wght":700}),

    # ── Latin ───────────────────────────────────────────────────────────────
    # Marcellus is Roman inscriptional capital — the other carved tradition.
    # Set beside Kufic it is not a contrast but a rhyme: both are letterforms
    # cut into stone rather than written with a pen.
    ("Marcellus",         "family=Marcellus",                 "latin",     400, None),
    ("Marcellus",         "family=Marcellus",                 "latin-ext", 400, None),
    # Archivo is the instrument layer: references, serials, classifications,
    # microtext. A grotesque built for small sizes, deliberately technical
    # against the two carved faces.
    ("Archivo",           "family=Archivo:wght@400",          "latin",     400, {"wght":400,"wdth":100}),
    ("Archivo",           "family=Archivo:wght@400",          "latin-ext", 400, {"wght":400,"wdth":100}),
    ("Archivo",           "family=Archivo:wght@600",          "latin",     600, {"wght":600,"wdth":100}),
    ("Archivo",           "family=Archivo:wght@600",          "latin-ext", 600, {"wght":600,"wdth":100}),
]
...
def main():
    ...
    header = """/* ══════════════════════════════════════════════════════════════════════════════
   THE PERSONAL OFFICE — INSTRUMENT TYPOGRAPHY
   ...
   font to install and no network request.
     ...
     Noto Kufi Arabic   Arabic display — the monumental, inscriptional hand
     Noto Naskh Arabic  Arabic text — letter bodies, addresses, the register
     Marcellus          Latin display — Roman inscriptional capital
     Archivo            Latin instrument — references, serials, microtext
