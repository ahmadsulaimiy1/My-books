...
def main():
    ...
    # the one move nothing else on the sheet makes, magnified: the name is
    # right-aligned to a point INSIDE the pier, so it stands in gold on sapphire
    # and in sapphire on pearl — one line of type, two materials
    px_mm = DPI / 25.4
    box = (int(96*px_mm), int(33*px_mm), int(210*px_mm), int(56*px_mm))
    det = detail(letter, box, 880)
