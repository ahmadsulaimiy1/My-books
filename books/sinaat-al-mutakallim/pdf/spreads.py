...
right and the odd page after it on the left. The case wrap is left out.

    python3 spreads.py ../Volume-I_Opening.pdf      writes ../book/_production/الإخراج/<name>_Spreads.pdf
"""
import sys
...
src = Path(sys.argv[1] if len(sys.argv) > 1 else Path(__file__).resolve().parent.parent / "Volume-I_Opening.pdf")
out_dir = Path(__file__).resolve().parent.parent / "book" / "_production" / "الإخراج"
out_dir.mkdir(parents=True, exist_ok=True)
d = pymupdf.open(str(src))
pages = list(range(1, len(d)))          # skip the case wrap
w, h = d[1].rect.width, d[1].rect.height
gap = 14
o = pymupdf.open()
