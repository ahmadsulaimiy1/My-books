#!/usr/bin/env python3
"""Build «الدليل التحريري والعلمي» — Editorial & Scholarly Bible — Edition 1.2 — PDF Master.
...
BIBLE = ROOT / "bible"
CACHE = HERE / ".cache"
OUT = ROOT / "Editorial-Scholarly-Bible_Edition-1.2_PDF-Master.pdf"
...
PARTS = [
    ...
    dict(n=7, file="07-دستور-الإصدار.md", word="السابع", en="Part Seven",
         title="دستور الإصدار",
         lede="الإتقان قبل الامتثال: كتابٌ مؤلَّف لا مولَّد، ومحرَّر لا مُجمَّع، ومصمَّم لا مُزيَّن، وموثَّق لا مُدّعى."),
    dict(n=8, file="08-مواصفة-مراجعة-المجلد-الأول.md", word="الثامن", en="Part Eight",
         title="مواصفة مراجعة المجلد الأول",
         lede="ما أظهره الفحص، وما يتغيّر، وكيف يُنفَّذ، وكيف تُفرز المجلدات الأخرى."),
    dict(n=9, file="09-نظام-التحقيق-والتوثيق.md", word="التاسع", en="Part Nine",
         title="نظام التحقيق والتوثيق",
         lede="لا نكتب كتابًا يبدو علميًّا؛ نكتب كتابًا يمكن تدقيقه علميًّا."),
    dict(n=10, file="10-المعمار-العلمي-للتأليف.md", word="العاشر", en="Part Ten",
         title="المعمار العلمي للتأليف",
         lede="لا تُجمِّل ما يحتاج إلى تحقيق، ولا تُحقِّق ما لا يحتاج إلى إطالة، ولا تجزم حيث لا يسمح الدليل."),
    dict(n=11, file="11-هندسة-المصادر-وأدوات-التحقيق.md", word="الحادي عشر", en="Part Eleven",
         title="هندسة المصادر وأدوات التحقيق",
         lede="لا يدخل اقتباسٌ تراثيٌّ مهمٌّ الطبعةَ النهائية بلا أصلٍ متحقّق؛ والأصل حذف غير المتحقّق."),
    dict(n=12, file="12-تنويع-المرجعية-وبناء-الهوامش.md", word="الثاني عشر", en="Part Twelve",
         title="تنويع المرجعية وبناء الهوامش",
         lede="لا تجعل الهوامش متأخرة عن الصفحة؛ اجعل الصفحة مصممة حول احتياجها العلمي إلى الهامش."),
    dict(n=13, file="13-منزلة-العربية-وطريق-التكوين.md", word="الثالث عشر", en="Part Thirteen",
         title="منزلة العربية وعلومها وطريق التكوين",
         lede="المسألة هي منزلة العربية، لا خطابةٌ في عظمتها؛ وقلّة العلوم الصحيحة خيرٌ من كثرة العناصر المصطنعة."),
    dict(n=14, file="14-النشر-والإخراج-وهوية-الدار.md", word="الرابع عشر", en="Part Fourteen",
         title="بنية النشر والمقدّمات وهوية الدار",
         lede="لا تنتج PDF جميلًا؛ أنتج كتابًا."),
    dict(n=15, file="15-هندسة-السلسلة-وعدد-المجلدات.md", word="الخامس عشر", en="Part Fifteen",
         title="هندسة السلسلة وعدد المجلدات",
         lede="عدد المجلدات تابعٌ للبنية العلمية، وليست البنية العلمية تابعةً لعدد المجلدات."),
    dict(n=16, file="16-النثر-العالم-والإغلاق.md", word="السادس عشر", en="Part Sixteen",
         title="النثر العالم، والإخراج المقصود، والإغلاق العلمي",
         lede="ليكن العلم هو الأصل، وليكن البيان خادمًا له؛ ولا يُحاوَل أن يكون كل سطرٍ بليغًا، بل يكون الكتاب كلّه بليغ البناء."),
]
...
PART_SUMMARIES = {
    ...
    7: ["الدليل إطارٌ مهنيٌّ حيّ يحكم المجلدات الأربعة، والإتقان مقدَّم على الامتثال.",
        "خمسة مفاهيم لا تُخلط: الصحة، والطبيعية، والمناسبة، والفصاحة، والبلاغة؛ ومستويات العربية على طيفٍ متصل.",
        "الكتاب للناطق بالعربية ولمتعلّمها، ونيجيريا حالةٌ من الحالات لا تعريفٌ للكتاب.",
        "لا وسم ولا لغة إنتاج في ملف النشر؛ والدعوى إما نقلٌ موثّق أو حقيقةٌ موثّقة أو إعادة صياغة أو رأي المؤلف.",
        "قانون الحرف ١٫٢: القاهري للكلمة الكبيرة، ومركزي للعناوين، وأميري للقراءة، وبلكس للتنقّل.",
        "الأصل هدوء الصفحة، والإطار أداةٌ لا لغة، والرسم حيث توجد علاقة؛ ولا زينة بلا وظيفة."],
    8: ["فحص المجلد الأول: نحو ١٨٥٠ إطارًا في ٨٤٥ صفحة، ولا رسم، و١٥٢ وسمًا ظاهرًا.",
        "مدخلٌ جديد قبل الجزء الأول: ما العربية التي نتكلّم بها؟",
        "معاملةٌ جديدة لكل عنصر، وسبعة عشر رسمًا مقرّرًا.",
        "خطةٌ في ست مراحل تنتهي بالبوابات الثماني، وعيّناتٌ مرجعية للنظام الجديد.",
        "المجلدات الأخرى تُفرز ولا يُعاد تأليفها."],
    9: ["كل نقلٍ يمرّ بسبع مراحل: العثور، والتحقق، والمطابقة، والتخريج، والمقارنة، والتوثيق، والمراجعة.",
        "لا حديث بلا تخريج، ولا تخريج بلا تحقّق، ولا نسبة إلى النبي ﷺ اعتمادًا على الذاكرة.",
        "لا تخمين في التوثيق: لا صفحة ولا رقم ولا نسبة إلا بعد المطابقة.",
        "طبعةٌ مرجعيةٌ واحدة لكل مصدر، وستة أنواعٍ من الحواشي، وحاشيةٌ مصمّمة لا مكدّسة.",
        "سجلٌّ مركزيٌّ بحالاتٍ معلنة، والبوابة التاسعة: لا يُطبع إلا المتحقّق.",
        "فإن قيل… قلنا؛ ولكل صيغةٍ من صيغ الاعتراض والجواب والترجيح وظيفة، ودرجة الجزم تساوي درجة الدليل.",
        "الإحالة الداخلية بالموضع لا بالصفحة، وفهارس علمية سبعة، وتسعة فحوصٍ قبل الطباعة.",
        "تنويع الصياغة لا يعني تنويع الشخصية: المؤلف واحد، ومقامات التأليف متعددة."],
    10: ["تحرير محلّ النزاع قبل الترجيح، ومعرفة متى لا نجادل.",
         "درجات اليقين معلنة، و«لا يصحّ» غير «لا أستحسن»، وستة أحكامٍ لغوية لا تُخلط.",
         "المصدر الأول قبل الثانوي، ولا «قال العلماء» بلا أسماء، ولا تسوية آليةً بين الأقوال.",
         "النص الشرعي لا يُحمَّل ما لا يدلّ عليه: دلالة، واستئناس، وفائدة تربوية.",
         "سجلّ المصطلحات، وسلّم اسم المؤلف، وأدوات المؤلف والمحقّق، وسجلّ التصحيحات."],
    11: ["اسم المؤلف الرسمي، وسلّمه النافذ في مواضع الكتاب.",
         "الطبعات المعتمدة قراراتٌ مكتوبة: الأصل، والمقابلة، وطريقة ما تختلفان فيه.",
         "الحالة غير المرحلة: لا «متحقّق» إلا ما طوبق على المطبوع نفسه، والمراجع الثاني شرطُ البوابة.",
         "قاعدة مصادر كبرى بأكثر من ثلاثمئة عنوان، في عشر مجموعات وثلاث طبقات، ولكل عنوانٍ وظيفة.",
         "خريطة نسب القول، وسجلّ الشواهد بدرجات الدلالة، ومعجم صيغ النقل، وفهرس النصوص الشرعية.",
         "اختبار الباحث العشوائي بعشرين إلى ثلاثين إحالة، واختبار المؤلف قبل كل طبعةٍ نهائية."],
    12: ["التوجيه الملزم بلفظ المؤلف: المسألة هي التي تستدعي العالم، لا العالم هو الذي يستدعي المسألة.",
         "خريطة الفنون وأهلها دليلٌ للبحث لا قائمةٌ للتوزيع، ولا رمز آليًّا لعمق العربية.",
         "الهامش جزءٌ من التأليف: اثنا عشر موضعًا تجب فيها الإحالة، ولا حاشية شكلية.",
         "النص، ثم التوثيق، ثم الهامش، ثم ضبط الصفحة، ثم الإخراج؛ وإن لم يُتحقَّق فالتحقيق أو الحذف أو التحليل.",
         "ثمانية أنواعٍ للحاشية بوسومها، وحواشي صفحةٍ حقيقية بمكتبة Paged.js.",
         "لا تُحذف حاشيةٌ لجمال الصفحة: يُعاد بناء المتن أو تُوزَّع المادة."],
    13: ["منزلة العربية في الشرع تثبت بالنص، وخصائصها بالوصف، ودعوى المقارنة بمنهجها.",
         "العربية ليست النحو وحده، والعالم بها ليس حافظ المتون وحده.",
         "حفظ الذكر مركزُ بقاء الفصحى، لا دعوى بحفظ كل تراثٍ وكل استعمال.",
         "محاولات الفصل عن الأصل تُذكر بجهتها وتاريخها ونصّها، وتُنقد بقوة، ولا تُعمَّم.",
         "خريطة العلوم تُظهر العلوم وحدها؛ وأبوابها ومسائلها تحتها، والمهارات خارجها."],
    14: ["دار الإحسان للتصميم والنشر ناشرًا، وهويتها في الصفحات التي تليق بها لا في كل صفحة.",
         "مقدّماتٌ حقيقية على الصفحات المتقابلة، لكل صفحةٍ وظيفتها وإيقاعها.",
         "الفخامة في التنويع المنضبط، لا في كثرة الزخارف؛ وأرضيةٌ لؤلؤية لا كريمية.",
         "أرقام أميري للصفحات والإحالات والحواشي والمباحث، بعد مقارنة الحروف الستة.",
         "لا يُعتمد تصميمٌ حتى يُختبر في الصفحات المتقابلة وفي الحالات الست عشرة."],
    15: ["الخريطة أولًا، ثم الحدود، ثم الانتقالات، ثم النقل، ثم بناء المجلد وعدّ صفحاته، ثم الكعب والغلاف.",
         "لكل مجلدٍ جملةٌ علميةٌ واحدة، ووظيفةٌ تضيع بحذفه، وحجمٌ مقيس لا مفروض، وحدٌّ يوافق المنهج.",
         "الثمانية قرارٌ سابق صار مقترحًا؛ وتوصية البوابة عشرة للمنهج ومرجع، حتى يدقّق المؤلف الحدود.",
         "أربعة أرقام لا يُخلط بينها: المجلد، والباب، والمستوى، والفصل؛ ورقم الباب ليس ترتيبه في المجلد.",
         "البنية العلمية ثابتة، والمادية تابعةٌ لها؛ ولا تُبقى بنيةٌ لمجرد أنها قُرّرت."],
    16: ["نثرٌ عالمٌ لا مزخرف: سجعٌ محكم في مواضعه، وتقسيمٌ وطباقٌ ومقابلة، وترادفٌ وظيفي، وعباراتٌ جامعةٌ قليلة.",
         "كلمة المؤلف تتدرّج من الله إلى الحاجة إلى الكتاب، وتنتهي إلى «البيان أمانة».",
         "أرقام الإحالة والحواشي تُقرأ بلا تكبير؛ ولا تُصغَّر الحاشية لملء الصفحة.",
         "القرآن طبقةٌ ياقوتيةٌ عميقة مستقلة، والحديث فحميٌّ بين خطّين ذهبيين، والأحمر لا يُستعمل زينة.",
         "العتبة والقاعدة: رأسٌ على خطٍّ ذهبيٍّ بوزنين تلتقيان عند علامةٍ لا تتغيّر، ورقم الصفحة على الخطّ نفسه مصغّرًا.",
         "الإغلاق العلمي: خمسة اختبارات لكل نقل، ومراجعةٌ مستقلة لكل يقينٍ فوق الدليل."],
}
...
APPROVAL_ROWS = [
    ...
    ("مقاس الكتاب", "٢٠×٢٦ للمجلدات الأحد عشر (١١٢ي)، ٢١×٢٨ سم"),
...
def mgmt_page():
    rows = [
        ...
        ("رمز الوثيقة", '<span class="ltr" style="font-family:var(--f-mono)">SMA-BIBLE-1.2</span>'),
        ("الإصدار", "١٫٢ (<span class=\"en\">Edition 1.2</span>) — <span class=\"en\">PDF Master</span>: نسخة رئيسية للطباعة والقراءة الرقمية"),
        ("تاريخ الإصدار", DATE_LINE),
        ("الحالة", f"معتمد مع قراراته العشرين (ص {pgref('sec-decisions')})؛ ومعدَّل في الإصدار ١٫١ بقانون الحرف وموجّه الغلاف، وفي الإصدار ١٫٢ بدستور الإصدار (الجزء السابع) ومواصفة مراجعة المجلد الأول (الجزء الثامن)"),
    ...
    policy = ("<ul><li><strong>١٫٠</strong> الإصدار الأول، واعتُمد مع قراراته العشرين.</li>"
              ...
              "<li><strong>١٫٢</strong> دستور الإصدار (الجزء السابع): الإتقان قبل الامتثال، وقانون الحرف ١٫٢، والإيقاع وأنواع الصفحات، والبوابات الثماني؛ ومواصفة مراجعة المجلد الأول وفرز المجلدات الأخرى (الجزء الثامن).</li>"
              "<li><strong>١٫٣…</strong> تعديلات جزئية لا تغيّر البنية، ومنها تسجيل القرارات بعد اعتمادها.</li>"
...
def assemble(font_css: str):
    ...
    doc = (f'<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8">'
           f'<title>الدليل التحريري والعلمي — صناعة المتكلّم العربي — الإصدار ١٫٢</title>'
...
def finalize(pdf_in: Path, titles: list, pdf_out: Path):
    ...
    writer.add_metadata({
        "/Title": "الدليل التحريري والعلمي — صناعة المتكلّم العربي — الإصدار ١٫٢",
        "/Subject": "Editorial & Scholarly Bible — Edition 1.2 — PDF Master",
        "/Author": "فريق مشروع «صناعة المتكلّم العربي»",
        "/Keywords": "صناعة المتكلّم العربي; الدليل التحريري والعلمي; Editorial & Scholarly Bible; Edition 1.2; PDF Master",
